...
router.route("/add").post((req,res)=>{
    ...
    if (Year > 2025) {
        return res.status(400).json({ error: "Year cannot be greater than 2025" });
    }
...
router.route("/update/:id").put(async(req, res)=>{
    ...
    if (Year < 0) {
        return res.status(400).send({status: "Year cannot be negative"});
    }
    if (Year > 2025) {
        return res.status(400).send({status: "Year cannot be greater than 2025"});
    }
